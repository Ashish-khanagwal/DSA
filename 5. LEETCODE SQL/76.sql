...
PROBLEM LINK -> https://leetcode.com/problems/analyze-subscription-conversion/
...
SELECT DISTINCT f.user_id,
    ROUND(
        SUM(CASE WHEN f.activity_type = 'free_trial' THEN f.activity_duration ELSE 0 END) / COUNT(CASE WHEN f.activity_type = 'free_trial' THEN 1 END), 
    2) AS trial_avg_duration,
    ROUND(
        SUM(CASE WHEN u.activity_type = 'paid' THEN u.activity_duration ELSE 0 END) / COUNT(CASE WHEN u.activity_type = 'paid' THEN 1 END), 
